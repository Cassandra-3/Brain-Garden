...
const SEASON_ACCENTS = {
  spring: { primary:"#4a7c59", secondary:"#6b9e78", glow:"#4a7c5940", name:"spring" },
  summer: { primary:"#b87333", secondary:"#c4622d", glow:"#b8733340", name:"summer" },
  autumn: { primary:"#8b2252", secondary:"#c4622d", glow:"#8b225240", name:"autumn" },
  winter: { primary:"#4a6a8a", secondary:"#6b8caa", glow:"#4a6a8a40", name:"winter" },
};

const SEASON = getSeason();
const SA = SEASON_ACCENTS[SEASON];
...
const C = {
  ...
  text:"#f0e6d3", gold:"#d4a843",
  season: SA.primary, seasonSoft: SA.secondary, seasonGlow: SA.glow,
};
...
// ── THEO THE CAT ─────────────────────────────────────────────────────
const THEO_MOODS = {
  sleepy:  { face:"(=_=)", note:"Theo is napping. maybe you should too. or maybe do one task first.", mood:"sleepy" },
  neutral: { face:"(•ᴗ•)", note:"Theo is watching. patiently.", mood:"neutral" },
  alert:   { face:"(=ᵒᴗᵒ=)", note:"Theo sees your effort and approves.", mood:"alert" },
  playful: { face:"(=^▽^=)", note:"Theo is doing zoomies. something good happened.", mood:"playful" },
  proud:   { face:"(=^◡^=)♡", note:"Theo is VERY proud of you right now.", mood:"proud" },
};

function getTheoMood(donePct, unlockedMysteries) {
  if (unlockedMysteries > 0 && donePct === 100) return THEO_MOODS.proud;
  if (donePct === 100) return THEO_MOODS.playful;
  if (donePct >= 50) return THEO_MOODS.alert;
  if (donePct >= 20) return THEO_MOODS.neutral;
  return THEO_MOODS.sleepy;
}

// ── DAILY MESSAGES (Hozier + Amble vibes, no actual lyrics) ──────────
const DAILY_MESSAGES = [
  "something in you already knows the way. trust it today.",
  "you are built from deeper things than a bad morning.",
  "Theo has decided today is yours. he rarely decides things for others.",
  "the work you do quietly is still work. it counts.",
  "there is a version of today that goes well. you're already in it.",
  "roots go down before anything grows up. you're in the roots phase.",
  "you carry more than people see. that's okay. you know it's there.",
  "the dark isn't empty. neither are you.",
  "small things done with intention are still sacred.",
  "Theo watched you sleep and decided you deserved a good day. he's usually right.",
  "you are not behind. you are exactly where the work is happening.",
  "something is growing in you that needs the quiet to finish forming.",
  "the jobs that are meant for you are looking for you too.",
  "even the moon takes time to go full. you're allowed to be in a phase.",
  "Theo says: do the thing. he will supervise.",
];

// ── HOZIER/AMBLE MILESTONE MESSAGES ──────────────────────────────────
const MILESTONE_MESSAGES = [
  "take me to church — except the church is your skincare routine and you just attended 🕯️",
  "work song energy. the days stack up. keep going.",
  "amble through it, unhurried. you're building something real.",
  "from eden, something grows. so do you.",
  "Theo has witnessed your progress and composed a small internal song about it.",
  "like a river, you find your way around every obstacle. always.",
];

// ── MYSTERIES ────────────────────────────────────────────────────────
const MYSTERIES = [
  { pts:10,  unlocked:false, icon:"🔮", title:"your first secret", color:C.burgundy,
    content:"okay so your gemini stellium in the 7th house? that's literally why you mentally draft 47 versions of every message before sending. AND why you're so good at seeing both sides. chaotic gift but it's a gift bestie 💫" },
  { pts:25,  unlocked:false, icon:"🍂", title:"deep autumn mode: unlocked", color:C.rust,
    content:"burgundy, rust, copper, forest green, wine. your colors. chosen by the universe and seasonal color analysis both. wear them. surround yourself. you look incredible in them. 🍂" },
  { pts:50,  unlocked:false, icon:"🌕", title:"virgo moon dispatch", color:C.copper,
    content:"virgo moon conjunct MC. your emotional security is tied to being competent and useful. when you feel anxious your brain says 'fix something.' you're not overthinking. you're moon-ing. there's a difference. 🌕" },
  { pts:75,  unlocked:false, icon:"🧶", title:"from the yarn vault", color:C.olive,
    content:"a cable sweater as your FIRST project. so scorpio rising of you. no beginner scarves. straight to the cables. frogging is not failing — every knitter frogs. the olive green is going to look stunning. 🧶" },
  { pts:100, unlocked:false, icon:"🐈‍⬛", title:"Theo has a confession", color:C.forest,
    content:"Theo has been here since the beginning. he chose you, not the other way around. he says the fiddle leaf fig (Fig) is doing well in the corner. he says you're doing well too. he doesn't say things like that lightly." },
  { pts:150, unlocked:false, icon:"🍮", title:"ras malai achievement", color:C.gold,
    content:"26 chenna discs. from scratch. you MADE THE CHEESE. the rabri is coming. this is not a small thing. you are a person who does impressive things quietly and that is the rarest kind. 🍮" },
  { pts:200, unlocked:false, icon:"🔐", title:"cancer sun in the 8th: the real tea", color:C.wine,
    content:"GOSSIP: cancer sun in the 8th house. you feel everything deeply and show almost none of it. people think you're chill. you are not chill. you are a slow-boiling ocean. this is why you're good at GRC — you sense hidden risks. that's not anxiety. that's your superpower in disguise. 🦀" },
  { pts:300, unlocked:false, icon:"🏆", title:"300 points. genuinely.", color:C.burgundy,
    content:"ISO 27001 auditors don't build themselves. GRC professionals don't grow on trees. you are building something real, from scratch, in a field that requires holding enormous complexity with grace. Wealthsimple, Rewind, whoever gets you — they're lucky. Theo agrees. 💼" },
];

const WEEK_SURPRISES = [
  "🎉 full week. done. Theo is doing slow blinks at you, which is cat for 'i love you and i'm proud.'",
  "🌟 everything. every task. the version of you from two months ago would be speechless.",
  "✨ complete week. deep autumn brain at full power. rest now.",
  "🍂 flawless. Theo has added a new leaf to Fig in your honour.",
];

const JOB_MILESTONES = [
  { count:1,  msg:"first application. the hardest one is always the first. Theo witnessed it. 📬" },
  { count:5,  msg:"5 applications. work song energy. keep stacking. 💪" },
...
const FACE_WASH_MSGS = [
  "5 face washes. your skin barrier is thriving and Theo approves 🌸",
  "10 face washes. you convinced yourself you didn't need to and did it anyway. that's everything. ✨",
  "15 face washes. this is a habit now. virgo moon is satisfied. 🌕",
];
...
const DEFAULT_TASKS = [
  ...
  { id:"w-laundry1",day:2, order:0, text:"Laundry step 1 👕 — put clothes in machine",     category:"home",      freq:"weekly",  time:"5 min",   energy:"low",    subtasks:["gather dirty clothes","check pockets","load machine + detergent","press start"] },
  { id:"w-restock", day:2, order:1, text:"Restock personal care supplies 💊",              category:"home",      freq:"monthly", time:"10 min",  energy:"low",    subtasks:["check toiletries","check skincare","check medicine cabinet","add to shopping list"] },
  ...
  { id:"w-laundry2",day:2, order:2, text:"Laundry step 2 👕 — move to dryer or hang dry",  category:"home",      freq:"weekly",  time:"5 min",   energy:"low",    subtasks:["move clothes to dryer or hang on stand"] },
  ...
  { id:"f-shower-4",day:4, order:0, text:"Hair wash shower 🚿💆 — morning so hair can dry", category:"selfcare",  freq:"daily",   time:"30 min",  energy:"medium", subtasks:["wet hair thoroughly","shampoo + massage scalp","condition for 3 min","rinse cold","towel dry","diffuse or air dry"] },
  ...
  { id:"f-declutter",day:4,order:2, text:"Declutter 🗂️ — one bag or box out of room",     category:"home",      freq:"monthly", time:"20 min",  energy:"high",   subtasks:["pick one small area","sort: keep / donate / bin","bag it up","move out of room today"] },
  ...
  { id:"f-shower-4b",day:4,order:4, text:"Shower 🚿",                                      category:"selfcare",  freq:"daily",   time:"15 min",  energy:"low",    subtasks:[] },
  ...
  { id:"s-skipprep",day:5, order:1, text:"Skip-level interview prep 🎯 — 1 question",      category:"jobsearch", freq:"weekly",  time:"15 min",  energy:"medium", subtasks:["pick a skip-level question","write your answer","focus on strategic thinking not just tasks"] },
  { id:"s-sheets",  day:5, order:2, text:"Change + wash bed sheets 🛏️",                   category:"home",      freq:"weekly",  time:"20 min",  energy:"medium", subtasks:["strip bed","put sheets in wash","remake bed with clean sheets"] },
  { id:"s-mirrors", day:5, order:2, text:"Wipe mirrors + windows in room 🪟",              category:"home",      freq:"biweekly",time:"10 min",  energy:"low",    subtasks:[] },
  { id:"s-deepclean",day:5,order:2, text:"Deep clean room 🧹 — baseboards + behind furniture", category:"home", freq:"monthly", time:"45 min",  energy:"high",   subtasks:["put on a playlist first","move furniture slightly — one piece at a time","vacuum behind + under","wipe baseboards","put back","reward yourself after"] },
...
function BurstParticles({ x, y, color, onDone }) {
  const particles = Array.from({length:10}, (_,i) => ({
    angle:(i/10)*Math.PI*2, dist:30+Math.random()*30,
  ...
  return (
    <div style={{position:"fixed",left:x,top:y,pointerEvents:"none",zIndex:500}}>
      {particles.map((p,i) => (
        <div key={i} style={{position:"absolute",width:p.size,height:p.size,borderRadius:"50%",background:p.color,transform:"translate(-50%,-50%)",animation:`burst${i} 0.6s ease-out forwards`}}>
          <style>{`@keyframes burst${i}{0%{transform:translate(-50%,-50%) translate(0,0) scale(1);opacity:1}100%{transform:translate(-50%,-50%) translate(${Math.cos(p.angle)*p.dist}px,${Math.sin(p.angle)*p.dist}px) scale(0);opacity:0}}`}</style>
        </div>
      ))}
    </div>
  );
...
function LockScreen({ onUnlock }) {
  ...
  const [stored, setStored] = useState(() => localStorage.getItem("bg-password") || "");
  const [setting, setSetting] = useState(!localStorage.getItem("bg-password"));
  ...
  const handleSet = () => {
    if (input.length < 3) { setError("at least 3 characters please"); return; }
    if (input !== confirm) { setError("they don't match — try again"); doShake(); return; }
    localStorage.setItem("bg-password", input);
    onUnlock();
  };

  const handleUnlock = () => {
    if (input === stored) { onUnlock(); }
    else { setError("not quite — try again"); setInput(""); doShake(); }
  };

  return (
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24}}>
      <div style={{fontSize:40,marginBottom:8}}>🐈‍⬛</div>
      <div style={{fontSize:18,fontWeight:900,color:C.cream,marginBottom:4}}>brain garden</div>
      <div style={{fontSize:12,color:C.muted,marginBottom:32}}>Theo is guarding the door.</div>

      <div style={{width:"100%",maxWidth:320,animation:shake?"shakeanim 0.4s ease":"none"}}>
        <style>{`@keyframes shakeanim{0%,100%{transform:translateX(0)}25%{transform:translateX(-8px)}75%{transform:translateX(8px)}}`}</style>

        {setting ? (
          <>
            <div style={{fontSize:12,color:C.muted,marginBottom:8}}>choose a password — a word, phrase, anything you'll remember</div>
            <input type="password" value={input} onChange={e=>setInput(e.target.value)} placeholder="your password" style={{width:"100%",background:C.surface,border:`1px solid ${C.border}`,borderRadius:10,color:C.text,fontSize:14,padding:"12px 14px",outline:"none",boxSizing:"border-box",marginBottom:10,fontFamily:"inherit"}} />
            <div style={{fontSize:12,color:C.muted,marginBottom:8}}>confirm it</div>
            <input type="password" value={confirm} onChange={e=>setConfirm(e.target.value)} placeholder="same again" style={{width:"100%",background:C.surface,border:`1px solid ${C.border}`,borderRadius:10,color:C.text,fontSize:14,padding:"12px 14px",outline:"none",boxSizing:"border-box",marginBottom:10,fontFamily:"inherit"}} onKeyDown={e=>e.key==="Enter"&&handleSet()} />
            {error && <div style={{fontSize:12,color:C.rust,marginBottom:8}}>{error}</div>}
            <button onClick={handleSet} style={{width:"100%",padding:"13px 0",borderRadius:10,border:"none",background:`linear-gradient(135deg,${C.burgundy},${C.copper})`,color:C.cream,fontWeight:800,fontSize:14,cursor:"pointer"}}>set password + enter 🌱</button>
          </>
        ) : (
          <>
            <input type="password" value={input} onChange={e=>setInput(e.target.value)} placeholder="password" style={{width:"100%",background:C.surface,border:`1px solid ${C.border}`,borderRadius:10,color:C.text,fontSize:14,padding:"12px 14px",outline:"none",boxSizing:"border-box",marginBottom:10,fontFamily:"inherit"}} onKeyDown={e=>e.key==="Enter"&&handleUnlock()} autoFocus />
            {error && <div style={{fontSize:12,color:C.rust,marginBottom:8}}>{error}</div>}
            <button onClick={handleUnlock} style={{width:"100%",padding:"13px 0",borderRadius:10,border:"none",background:`linear-gradient(135deg,${C.burgundy},${C.copper})`,color:C.cream,fontWeight:800,fontSize:14,cursor:"pointer"}}>enter 🐈‍⬛</button>
          </>
        )}
      </div>
    </div>
...
// ── MOOD CHECK-IN ────────────────────────────────────────────────────
function MoodCheckIn({ type, onComplete }) {
  const [energy, setEnergy] = useState(3);
  const [anxiety, setAnxiety] = useState(3);
  const [word, setWord] = useState("");
  const [morningMed, setMorningMed] = useState(null);
  const [eveningMed, setEveningMed] = useState(null);

  const sliderStyle = (val, max=5) => ({
    width:"100%", height:6, borderRadius:3, background:C.border, position:"relative", margin:"8px 0 16px",
    cursor:"pointer",
  });

  const Slider = ({ value, onChange, max=5, color }) => (
    <div style={{position:"relative",marginBottom:16}}>
      <input type="range" min={1} max={max} value={value} onChange={e=>onChange(parseInt(e.target.value))}
        style={{width:"100%",accentColor:color||C.gold,cursor:"pointer"}} />
      <div style={{display:"flex",justifyContent:"space-between",fontSize:10,color:C.muted,marginTop:2}}>
        <span>1</span><span style={{color:color||C.gold,fontWeight:700}}>{value}</span><span>{max}</span>
      </div>
    </div>
  );
  ...
  const handleSubmit = () => {
    const entry = {
      type, date:new Date().toISOString(), energy, anxiety, word,
      morningMed: type==="midday" ? morningMed : undefined,
      eveningMed: type==="evening" ? eveningMed : undefined,
    };
    const existing = JSON.parse(localStorage.getItem("bg-mood") || "[]");
    existing.push(entry);
    localStorage.setItem("bg-mood", JSON.stringify(existing.slice(-60)));
    onComplete(entry);
  };

  return (
    <div style={{position:"fixed",inset:0,background:"#00000095",zIndex:800,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div style={{maxWidth:380,width:"100%",background:C.card,border:`2px solid ${SA.primary}`,borderRadius:20,padding:24,maxHeight:"90vh",overflowY:"auto"}}>
        <div style={{fontSize:24,marginBottom:4}}>🐈‍⬛</div>
        <div style={{fontSize:15,fontWeight:800,color:C.cream,marginBottom:2}}>
          {type==="midday" ? "midday check-in" : "evening check-in"}
        </div>
        <div style={{fontSize:12,color:C.muted,marginBottom:20}}>Theo wants to know how you're doing.</div>

        {/* medication */}
        {type==="midday" && (
          <div style={{marginBottom:20,background:C.surface,borderRadius:12,padding:14,border:`1px solid ${C.border}`}}>
            <div style={{fontSize:12,fontWeight:700,color:C.copper,marginBottom:10}}>💊 morning pill — did you take it?</div>
            <div style={{display:"flex",gap:8}}>
              {["yes ✓","not yet","i'll take it now"].map(o => (
                <button key={o} onClick={()=>setMorningMed(o)} style={{flex:1,padding:"8px 4px",borderRadius:8,border:`1px solid ${morningMed===o?C.gold:C.border}`,background:morningMed===o?`${C.gold}22`:"transparent",color:morningMed===o?C.gold:C.muted,fontSize:11,fontWeight:700,cursor:"pointer"}}>{o}</button>
              ))}
            </div>
          </div>
        )}
        {type==="evening" && (
          <div style={{marginBottom:20,background:C.surface,borderRadius:12,padding:14,border:`1px solid ${C.border}`}}>
            <div style={{fontSize:12,fontWeight:700,color:C.copper,marginBottom:10}}>💊 evening pill — did you take it?</div>
            <div style={{display:"flex",gap:8}}>
              {["yes ✓","not yet","i'll take it now"].map(o => (
                <button key={o} onClick={()=>setEveningMed(o)} style={{flex:1,padding:"8px 4px",borderRadius:8,border:`1px solid ${eveningMed===o?C.gold:C.border}`,background:eveningMed===o?`${C.gold}22`:"transparent",color:eveningMed===o?C.gold:C.muted,fontSize:11,fontWeight:700,cursor:"pointer"}}>{o}</button>
              ))}
            </div>
          </div>
        )}

        <div style={{fontSize:12,fontWeight:700,color:C.muted,marginBottom:4}}>energy level</div>
        <Slider value={energy} onChange={setEnergy} color={C.olive} />

        <div style={{fontSize:12,fontWeight:700,color:C.muted,marginBottom:4}}>anxiety level</div>
        <Slider value={anxiety} onChange={setAnxiety} color={C.burgundy} />

        <div style={{fontSize:12,fontWeight:700,color:C.muted,marginBottom:6}}>one word for right now</div>
        <input value={word} onChange={e=>setWord(e.target.value)} placeholder="anything — foggy, okay, tired, alive..." maxLength={20}
          style={{width:"100%",background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,color:C.text,fontSize:13,padding:"10px 12px",outline:"none",fontFamily:"inherit",boxSizing:"border-box",marginBottom:16}} />

        <button onClick={handleSubmit} style={{width:"100%",padding:"12px 0",borderRadius:10,border:"none",background:`linear-gradient(135deg,${C.burgundy},${C.copper})`,color:C.cream,fontWeight:800,fontSize:14,cursor:"pointer"}}>
          done 🐈‍⬛
        </button>
      </div>
    </div>
...
function MoodHistory() {
  const entries = JSON.parse(localStorage.getItem("bg-mood") || "[]").slice(-14).reverse();
  if (!entries.length) return <div style={{fontSize:13,color:C.muted,padding:"20px 0"}}>no check-ins yet — they'll appear here after your first one 🌙</div>;

  const energyColor = (v) => v >= 4 ? C.olive : v >= 3 ? C.copper : C.burgundy;
  const anxColor    = (v) => v <= 2 ? C.olive : v <= 3 ? C.copper : C.burgundy;
      ...
      {entries.map((e,i) => (
        <div key={i} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,padding:14,marginBottom:8}}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
            <div style={{fontSize:11,color:C.muted}}>{new Date(e.date).toLocaleDateString("en-CA",{weekday:"short",month:"short",day:"numeric"})} · {e.type}</div>
            {e.word && <div style={{fontSize:11,color:C.gold,fontWeight:700}}>"{e.word}"</div>}
          </div>
          <div style={{display:"flex",gap:12}}>
            <div style={{fontSize:11,color:energyColor(e.energy)}}>⚡ energy {e.energy}/5</div>
            <div style={{fontSize:11,color:anxColor(e.anxiety)}}>🌀 anxiety {e.anxiety}/5</div>
          </div>
          {e.morningMed && <div style={{fontSize:11,color:C.copper,marginTop:4}}>💊 morning pill: {e.morningMed}</div>}
          {e.eveningMed && <div style={{fontSize:11,color:C.copper,marginTop:4}}>💊 evening pill: {e.eveningMed}</div>}
        </div>
      ))}
    </div>
  );
...
function EnergyBadge({ energy, time }) {
  const e = ENERGY[energy||"low"];
  return (
    <div style={{display:"flex",gap:6,alignItems:"center",marginTop:5,flexWrap:"wrap"}}>
      <span style={{fontSize:10,fontWeight:700,color:e.color,background:e.bg,border:`1px solid ${e.color}`,borderRadius:4,padding:"1px 6px"}}>{e.dot} {e.label}</span>
...
function TaskCard({ task, onToggle, onMove, onDelete }) {
  ...
  const handleToggle = (e) => {
    if (!task.done) {
      ...
      setRing(true); setTimeout(()=>setRing(false),400);
      playDoneSound();
    }
    onToggle(task.id);
  };
    ...
    <>
      ...
      <div style={{background:task.done?C.surface:cat.bg,border:`1px solid ${task.done?C.border:cat.border}`,borderRadius:10,padding:"10px 12px",marginBottom:6,opacity:task.done?0.5:1,transition:"all 0.2s"}}>
        <div style={{display:"flex",alignItems:"flex-start",gap:8}}>
          <button onClick={handleToggle} style={{width:20,height:20,borderRadius:"50%",border:`2px solid ${cat.dot}`,background:task.done?cat.dot:"transparent",cursor:"pointer",flexShrink:0,marginTop:1,display:"flex",alignItems:"center",justifyContent:"center",boxShadow:ring?`0 0 0 6px ${cat.dot}55`:"none",transform:ring?"scale(1.3)":"scale(1)",transition:"transform 0.15s,box-shadow 0.15s"}}>
          ...
          <div style={{flex:1,minWidth:0}}>
            ...
            <EnergyBadge energy={task.energy} time={task.time} />
                ...
                {open && (
                  <div style={{marginTop:4,background:C.surface,borderRadius:8,padding:"8px 10px"}}>
                    {task.subtasks.map((s,i)=>(
                      <div key={i} style={{fontSize:12,color:C.muted,padding:"3px 0",borderBottom:i<task.subtasks.length-1?`1px solid ${C.border}`:"none",display:"flex",gap:6}}>
        ...
        <div style={{marginTop:5}}>
          <span style={{fontSize:10,fontWeight:700,color:cat.dot,background:cat.bg,border:`1px solid ${cat.border}`,borderRadius:4,padding:"1px 6px",textTransform:"uppercase"}}>{cat.emoji} {cat.label}</span>
        </div>
      </div>
    </>
  );
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"#00000095",zIndex:900,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div style={{maxWidth:400,width:"100%",background:C.card,border:`2px solid ${C.gold}`,borderRadius:20,padding:24,maxHeight:"90vh",overflowY:"auto"}}>
        ...
        {!perfect&&pct<70&&<div style={{marginTop:16,fontSize:13,color:C.muted,lineHeight:1.5}}>rough week. it happens. you don't lose points for hard weeks. 🌱</div>}
...
export default function App() {
  const [unlocked, setUnlocked] = useState(!!localStorage.getItem("bg-password") === false ? false : false);
  const [authed, setAuthed] = useState(false);

  // check if password exists — if not, go straight to setup
  const hasPassword = !!localStorage.getItem("bg-password");
...
function MainApp() {
  ...
  const [showMood, setShowMood]     = useState(null); // "midday" | "evening" | null
  ...
  useEffect(() => {
    ...
    const moodLog = JSON.parse(localStorage.getItem("bg-mood")||"[]");
    const todayEntries = moodLog.filter(e => new Date(e.date).toDateString()===today);
    const hasMidday = todayEntries.some(e=>e.type==="midday");
    const hasEvening = todayEntries.some(e=>e.type==="evening");
    if (hour>=12 && hour<18 && !hasMidday) { setTimeout(()=>setShowMood("midday"), 2000); }
    else if (hour>=18 && !hasEvening) { setTimeout(()=>setShowMood("evening"), 2000); }
  }, []);
  ...
  const toggleTask = (id) => {
    ...
    if (completing) {
      setPoints(p=>p+pts);
      const milestone=MILESTONE_MESSAGES[Math.floor(Math.random()*MILESTONE_MESSAGES.length)];
      showToast(`+${pts} pts ✨`);
  ...
  const handleAI = async () => {
    ...
    try {
      ...
      const prompt=isJob
        ?`Help someone with ADHD break a job search task into tiny concrete steps. Task: "${aiInput}". Return ONLY valid JSON: {"taskName":"short name","subtasks":["step 1","step 2","step 3"],"suggestedDay":0,"time":"X min","energy":"low|medium|high"}. suggestedDay 0-6 Mon=0. Max 5 steps each under 15 min.`
        :`Help someone with ADHD add a task to their weekly planner. Task: "${aiInput}", Category: ${aiCat}. Return ONLY valid JSON: {"taskName":"short name","subtasks":[],"suggestedDay":${addDay},"time":"X min","energy":"low|medium|high"}`;
  ...
  const theo=getTheoMood(pct,unlockedCount);

  const navItems=[{id:"today",icon:"⚡",label:"today"},{id:"week",icon:"📅",label:"week"},{id:"add",icon:"＋",label:"add"},{id:"mood",icon:"🌙",label:"mood"},{id:"secrets",icon:"🔮",label:"secrets"}];

  return (
    <div style={{background:C.bg,minHeight:"100vh",fontFamily:"'Inter',system-ui,sans-serif",color:C.text,paddingBottom:80}}>
      {toast && <div style={{position:"fixed",top:16,left:"50%",transform:"translateX(-50%)",background:C.gold,color:C.bg,padding:"10px 20px",borderRadius:12,fontWeight:800,fontSize:13,zIndex:1000,boxShadow:"0 4px 24px #00000080",maxWidth:"88vw",textAlign:"center",lineHeight:1.5}}>{toast}</div>}
      ...
      {showMood && <MoodCheckIn type={showMood} onComplete={()=>{ showToast("check-in saved 🐈‍⬛"); setShowMood(null); }} />}

      {/* Header */}
      <div style={{padding:"20px 20px 0",borderBottom:`1px solid ${C.border}`,paddingBottom:14}}>
        {/* daily message */}
        <div style={{fontSize:11,color:SA.primary,fontStyle:"italic",marginBottom:12,lineHeight:1.5,borderLeft:`2px solid ${SA.primary}`,paddingLeft:10}}>{dailyMsg}</div>

        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          <div>
            {/* Theo */}
            <div style={{fontSize:14,fontFamily:"monospace",color:C.gold,marginBottom:2}}>{theo.face}</div>
            <div style={{fontSize:10,color:C.muted,marginBottom:4}}>Theo · {theo.mood}</div>
            <div style={{fontSize:11,color:C.muted,fontStyle:"italic",marginBottom:2,maxWidth:200,lineHeight:1.4}}>{theo.note}</div>
            <div style={{fontSize:11,color:C.muted,marginTop:4}}>{FULL_DAYS[todayIdx]} · {SEASON} 🍂</div>
          </div>
          <div style={{textAlign:"right"}}>
            ...
            {nextM && <div style={{fontSize:10,color:C.muted,marginTop:1}}>{nextM.pts-points} to next 🔒</div>}
          </div>
        </div>
      ...
      <div style={{padding:20}}>
          ...
          <div style={{maxWidth:500}}>
            ...
            {todayTasks.length>0 && (
              <div style={{marginBottom:16}}>
                ...
                {pct===100 && <div style={{fontSize:13,color:C.gold,fontWeight:700,marginTop:6}}>🎉 everything done. Theo is doing a slow blink at you.</div>}
              </div>
            )}
            {[{label:"🌅 morning",orderVals:[0]},{label:"☀️ daytime",orderVals:[1,2]},{label:"🌙 evening",orderVals:[3,4]}].map(group=>{
        ...
        {view==="add" && (
          <div style={{maxWidth:500}}>
            ...
            <div style={{fontSize:12,color:C.muted,marginBottom:16}}>type anything — ai breaks it down, estimates time + energy, schedules it</div>
            ...
            <textarea value={aiInput} onChange={e=>setAiInput(e.target.value)} placeholder={aiCat==="jobsearch"?"e.g. prepare for a skip-level interview":aiCat==="selfcare"?"e.g. start a hair care routine":aiCat==="home"?"e.g. reorganise my wardrobe":"e.g. call the dentist"} rows={3}
        ...
        {view==="mood" && (
          <div style={{maxWidth:500}}>
            <div style={{fontSize:15,fontWeight:800,marginBottom:4}}>🌙 mood log</div>
            <div style={{fontSize:12,color:C.muted,marginBottom:16}}>your last 14 check-ins</div>
            <div style={{display:"flex",gap:8,marginBottom:20}}>
              <button onClick={()=>setShowMood("midday")} style={{flex:1,padding:"10px 0",borderRadius:10,border:`1px solid ${C.copper}`,background:`${C.copper}18`,color:C.copper,fontWeight:700,fontSize:12,cursor:"pointer"}}>+ midday check-in</button>
              <button onClick={()=>setShowMood("evening")} style={{flex:1,padding:"10px 0",borderRadius:10,border:`1px solid ${C.burgundy}`,background:`${C.burgundy}18`,color:C.burgundy,fontWeight:700,fontSize:12,cursor:"pointer"}}>+ evening check-in</button>
            </div>
            <MoodHistory />
          </div>
        )}
        ...
        {view==="secrets" && (
          <div style={{maxWidth:500}}>
            ...
            <div style={{fontSize:12,color:C.muted,marginBottom:16}}>{unlockedCount} of {mysteries.length} unlocked · {points} pts total</div>
      ...
      <div style={{position:"fixed",bottom:0,left:0,right:0,background:C.surface,borderTop:`1px solid ${C.border}`,display:"flex",justifyContent:"space-around",padding:"10px 0 14px",zIndex:100}}>
        {navItems.map(n=>(
          <button key={n.id} onClick={()=>setView(n.id)} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:2,background:"none",border:"none",cursor:"pointer",color:view===n.id?C.gold:C.muted,fontWeight:view===n.id?800:400}}>
            ...
            <span style={{fontSize:9,letterSpacing:"0.05em"}}>{n.label}</span>
            {n.id==="secrets"&&unlockedCount>0&&<span style={{fontSize:8,color:C.gold,fontWeight:900}}>{unlockedCount} unlocked</span>}
          </button>
        ))}
      </div>
    </div>
